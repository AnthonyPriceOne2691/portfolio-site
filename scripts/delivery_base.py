...
TEST_TEXT_SUFFIXES = (
    ...
    ".go", ".rs", ".swift", ".kt", ".java", ".rb", ".php", ".cs", ".md",
)
# ⚠ `.mjs`/`.cjs`/`.mts`/`.cts` добавлены по замеру. Список знал `.js`, но не
# ESM-расширения — а весь тестовый слой этого проекта лежит в `tests/*.test.mjs`.
# Гейт читал НОЛЬ тестовых файлов и объявлял все acceptance-примеры (B2…B11)
# «без ссылки в тестах», хотя ссылки в них есть. Здесь класс дал не ложное
# зелёное, а ложную тревогу — но корень тот же, что у L3 и L6 в
# `delivery/archive/INDEX.md`: маска канона не знает раскладку проекта, и гейт
