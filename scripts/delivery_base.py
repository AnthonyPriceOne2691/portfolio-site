...
BREAKER_EXCLUDE = (
    ...
    ".github/workflows/",
    ".claude/",
    # Lock-файлы — СГЕНЕРИРОВАННОЕ, а не авторское. Замер: поставка mvp-foundation
    # дала 870 строк, из них 564 — `package-lock.json` от четырёх devDependency;
    # лимит 800 оказался пробит до написания первой строки вёрстки.
    #
    # Считать их в blast radius неверно не только арифметически. Объём lock'а —
    # функция числа пакетов, а не риска правки, и построчно его не читает никто.
    # Риск новых зависимостей уже судит ДРУГОЙ гейт, точнее нацеленный:
    # `check_new_dependency.py` требует строку `new_dependency:` в STATUS на
    # каждую ПРЯМУЮ зависимость с причиной и автором.
    #
    # Хуже того, прежний счёт создавал вредный стимул: дешевле не объявлять
    # зависимость, чем объявлять. Ровно так в этом проекте и появился
    # транзитивный `prettier`, на котором молча стоял гейт формата.
    #
    # Находка канона (BACKLOG): список исключений собран по принципу «механика
    # контура», а сюда просится второй принцип — «сгенерированное, а не
    # написанное». Под него же попадут `poetry.lock`, `yarn.lock`, `Cargo.lock`.
    "package-lock.json",
    "yarn.lock",
    "pnpm-lock.yaml",
    "poetry.lock",
