...
no_ci_evidence() { # $1 = причина, которую увидел гейт
  ...
  [[ -f "$st" ]] || return 0
  # ⚠ `\**` ПОСЛЕ двоеточия — не косметика регекспа. Канонный STATUS пишет
  # `- **ci-oracles:** deployed`, то есть двоеточие ВНУТРИ жирного, а прежний
  # паттерн ждал `**ci-oracles**:`. Строка при этом совпадала, но `\([a-z]*\)`
  # упирался в `*` и захватывал ПУСТОТУ — дальше срабатывал выход «строки нет,
  # это не наш дефект», и гейт мягко пропускал ЛЮБОЕ значение. То есть ветка,
  # ради которой функция написана («в STATUS deployed, а CI не подтверждён —
  # красней»), не срабатывала никогда. Найдено 07.08 на первом же живом мерже:
  # merge_guard напечатал «gh не авторизован» и следом «OK».
  # Что это доказывает про канон: над одним файлом живут ДВА разбора, и
  # `delivery_check.py` ту же строку читает верно — расхождение парсеров внутри
  # одного контура. Находка в BACKLOG: разбор STATUS обязан быть один.
...
# ⚠ Проверка авторизации — `gh auth token`, а НЕ `gh auth status`. Второй ходит
...
# порядке, а лежала сеть. Диагноз по симптому — тот же класс, что с `npm install`
# 04.08 (см. active/escalation.md): «реестр недоступен» оказалось деградацией
# канала. `gh auth token` читает сохранённые учётки локально, поэтому различает
# «учёток нет» и «не дозвонились»; второе всплывёт ниже, на реальном вызове API,
# и уже со своим текстом.
