...
if [[ -n "$DC_CFG" ]]; then
  ...
  if [[ -z "$DC" ]]; then
    ...
    # ⚠ Скобка НЕ обязательна, и это стоило ложного диагноза. depcruise печатает
    # счётчик в двух формах: «✔ no dependency violations found (9 modules, …)» —
    # со скобкой, и «x 2 dependency violations (0 errors, 2 warnings). 9 modules,
    # … cruised.» — где скобка принадлежит числу ошибок, а не модулей. Прежний
    # шаблон `\([0-9]+ modules` находил только первую: стоило появиться ОДНОМУ
    # предупреждению, как счётчик обнулялся и гейт объявлял «0 модулей
    # просмотрено — гейт не видел кода». Это хуже красного: диагноз отправлял
    # чинить область поиска, а с областью всё было в порядке.
