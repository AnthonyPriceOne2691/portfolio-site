...
#
# ⚠ `tests/*` без ведущего слэша и `*.test.mjs` добавлены после замера, а не
# для полноты. Маска `*/tests/*` требует каталог ПЕРЕД `tests`, поэтому пути
# вида `tests/layout.test.mjs` (тесты в корне репозитория — обычная раскладка
# для фронта) под неё не подходили, а `*.test.ts` не знает про `.mjs`. Весь
# тестовый слой проекта судился по прод-лимиту 500 вместо 1000: гейт краснел на
# файле, который в свой лимит укладывался вдвое. Тот же класс, что маска, не
# знавшая про `.astro` (cqg@1.69), — шаблон канона нёс чужую раскладку.
is_test() {
  case "$1" in
    *.test.ts | *.test.tsx | *.test.mjs | *.test.js) return 0 ;;
