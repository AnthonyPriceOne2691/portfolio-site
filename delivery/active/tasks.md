...
- [ ] T1. `.astro` под линтер и форматтер: `eslint-plugin-astro` +
      ...
      **Плюс объявить `prettier` прямой devDependency:** сейчас он транзитивный,
      то есть гейт формата стоит на пакете, которого никто не гарантирует
      (найдено 07.08 при накате cqg@1.91)
...
- [ ] T7a. **Оракул артефакта** (§6.5a): скрипт над `dist/` — состав обеих
      языковых веток и `<head>` каждой страницы (og, hreflang, JSON-LD).
      Закрывает B1 и B8 машинно и снимает `artifact_oracle: n/a` в STATUS;
      к handoff строка обязана стать путём к файлу
