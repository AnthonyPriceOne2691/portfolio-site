...
- **artifact_oracle:** n/a reason=скрипт написан и проверен, приезжает мержем
  4/4 этой же поставки; объявлять путь раньше файла гейт справедливо не даёт
- **runtime_paths:** src/
...
- **baseline_growth_waiver:** reason=снимок DRY поднят 0 -> 2; остаток — каркасный обвяз Astro (импорт, interface Props, деструктуризация), неустранимый примесями: фреймворк требует объявлять их в каждом компоненте. Настоящие дубли гейт нашёл, и они ВЫНЕСЕНЫ, а не узаконены — lib/projects.ts (запрос жил в трёх местах), ProjectGrid.astro (сетка на двух страницах), PageShell.astro (вызов Base на трёх). Рост сверх двух снова покраснеет by=human:anthony
- **observability:** 1
