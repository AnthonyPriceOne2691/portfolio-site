...
- **artifact_oracle:** n/a reason=скрипт написан и проверен, но приезжает
  последним мержем этой же поставки — поставка режется на четыре части, потому
  что целиком пробила circuit breaker (42 файла / 1687 строк против 25 / 800).
  Объявлять путь раньше файла нельзя: гейт справедливо зовёт это «снятой
  проверкой с видом усиления» (§4.6). Строка станет путём в мерже 4/4
- **runtime_paths:** src/
