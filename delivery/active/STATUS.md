...
- **artifact_oracle:** scripts/lint/check_dist_oracle.py
  <!-- §6.5a закрыт по факту, а не обещанием. Судит СОБРАННЫЙ сайт: состав
       обеих языковых веток и зеркальность страниц (B1), мета-слой каждой
       страницы — og/hreflang/x-default/JSON-LD (B8), вес без медиа < 300 KB
       (B10). Фальсифицирован тремя способами: убранный og:image, x-default,
       уведённый на EN, и пропавшая EN-страница — каждый даёт красное с
       адресным диагнозом. Подключён шагом CI после `npm run build` и строкой
       `stages: [manual]` в pre-commit, чтобы gate-coverage видел его гейтом,
       а не забытым скриптом. -->
- **runtime_paths:** src/
