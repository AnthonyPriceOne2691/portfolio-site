...
3. **Прогон мониторинга #1 висит в «Running» 88 часов, #9 — в «Queued» с
   22.09.** Это первые строки таблицы «Recent runs». Либо поправить в базе,
   либо не прокручивать до таблицы.
...
- **Senders.** У большинства доменов 13–16% отказов при статусе RUNNING: рядом
  с любой фразой про паузу по отказам это противоречие.
...
**Блокер: фикстура с формой заказа (порт 8908) — русская.** На странице
«Оплатить заказ» и «4 900 ₽». Для английского ролика нужна английская копия:
27 строк HTML отдельным сайтом на порту 8909. Английские сигналы необратимости
(`pay`, `place order`, `checkout`) контракт уже знает, а тесты, которые держатся
за русскую фикстуру, не трогаются. Без копии — запасной ключевой кадр, см.
«Если пойдёт не так».
...
- **Сессии для витрины.** Основная — магазины электроники (adafruit / pihut /
  sparkfun). Немецкую про хлеб текстом не открывать: в немецкий ответ протёк
  внутренний ключ `brot_backen_anleitung_url`. `report.md` King Arthur не
